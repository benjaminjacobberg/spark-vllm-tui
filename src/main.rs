...
use app::{Action, App, AppState, ClusterStatus, ConnectState, StatusUpdate};
...
async fn main() -> Result<()> {
    ...
    let config = AppConfig::load_from_file("server.json").unwrap_or_else(|_| AppConfig::default());

    // Load model definitions
    let models = load_models("models.json").unwrap_or_else(|_| vec![]);
            ...
            match update {
                StatusUpdate::ConnectState(state, msg) => {
                    if let ConnectState::Failed(ref e) = state {
                        // Stay in Login state, show error
                        app.status_message = format!("Error: {}", e);
                    } else if let ConnectState::Connected = state {
    ...
    tokio::task::spawn_blocking(move || {
        ...
        if let Some(client) = guard.as_ref() {
            ...
            match client.run_command(&cmd) {
                Ok((_, _, code)) => {
                    if code == 0 {
                        let _ = status_tx.blocking_send(StatusUpdate::ClusterStatus(
                            ClusterStatus::Running,
                            format!("Session {} is active.", session_name),
                        ));
                    } else {
                        let _ = status_tx.blocking_send(StatusUpdate::ClusterStatus(
                ...
                Err(e) => {
                    let _ = status_tx
                        .blocking_send(StatusUpdate::Message(format!("Status check error: {}", e)));
                }
            }
        }
    });
...
async fn handle_stop_cluster(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        if let Some(client) = guard.as_ref() {
            let stop_cmd = format!(
                "cd {} && ./launch-cluster.sh --nodes \"{}\" stop",
                config.vllm_docker_dir, nodes_arg
            );
            ...
            match res {
                Ok((_, _, _)) => {
                    ...
                    let _ = status_tx.blocking_send(StatusUpdate::ClusterStatus(
                        ClusterStatus::Stopped,
                        "Cluster Stopped.".into(),
                    ));
                    is_tailing.store(false, std::sync::atomic::Ordering::Relaxed);
                }
                Err(e) => {
                    let _ =
                        log_tx.blocking_send(LogMessage::Line(format!("Error stopping: {}", e)));
...
async fn handle_start_cluster(
    ...
    if let Some(model) = app.selected_model() {
        ...
        app.status_message = format!(
            "Starting cluster with model {} (Session: {})...",
            model.name, session_name
        );
        let _ = log_tx
            .send(LogMessage::Line(format!("CMD: {}", tmux_cmd)))
            .await;
        ...
        tokio::task::spawn_blocking(move || {
            ...
            if let Some(client) = guard.as_ref() {
                match client.run_command(&tmux_cmd) {
                    Ok((out, err, code)) => {
                        let _ = log_tx.blocking_send(LogMessage::Line(format!(
                            "Exited with {}. Out: {}. Err: {}",
                            code, out, err
                        )));
                        let _ = status_tx.blocking_send(StatusUpdate::ClusterStatus(
                            ClusterStatus::Running,
                            "Cluster Started (Tmux session created).".into(),
                        ));
                    }
                    Err(e) => {
                        let _ = log_tx
                            .blocking_send(LogMessage::Line(format!("Execution Error: {}", e)));
                        let _ =
                            status_tx.blocking_send(StatusUpdate::Message("Start Error.".into()));
                    }
                }
            }
        });
        ...
        loop {
            ...
            if let Some(client) = guard.as_ref() {
                match client.run_command(&cmd) {
                    ...
                    Err(e) => {
                        let _ =
                            log_tx.blocking_send(LogMessage::Line(format!("Watch error: {}", e)));
                    }
                }
            } else {
                break;
...
async fn handle_attach_session(app: &mut App, log_tx: mpsc::Sender<LogMessage>) {
    ...
    if let Err(e) = status {
        let _ = log_tx
            .send(LogMessage::Line(format!("Attach failed: {}", e)))
            .await;
    }
...
mod tests {
    ...
    fn test_get_session_name() {
        ...
        assert_eq!(
            get_session_name("DeepSeek-V3 (AWQ)"),
            "vllm-deepseek-v3-awq"
        );
    }
}
